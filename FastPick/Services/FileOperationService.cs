...
public static class FileOperationService
{
    ...
    /// <returns>成功导出的文件数量</returns>
    public static async Task<int> ExportPhotosAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (options.ExportOption == ExportOptionEnum.JpgOnly || options.ExportOption == ExportOptionEnum.Both)
        {
            Directory.CreateDirectory(jpgFolder);
        }
        if (options.ExportOption == ExportOptionEnum.RawOnly || options.ExportOption == ExportOptionEnum.Both)
        {
            Directory.CreateDirectory(rawFolder);
        }

        // 筛选符合评级要求的图片
        var filteredPhotos = photos.Where(p => p.Rating >= options.MinRating).ToList();
        
        var exportProgress = new ExportProgress
        {
            TotalFiles = filteredPhotos.Count
        };
        ...
        await Task.Run(() =>
        {
            foreach (var photo in filteredPhotos)
            {
                ...
                try
                {
                    // 导出 JPG
                    if ((options.ExportOption == ExportOptionEnum.JpgOnly || options.ExportOption == ExportOptionEnum.Both) 
                        && !string.IsNullOrEmpty(photo.JpgPath) && File.Exists(photo.JpgPath))
                    {
                    ...
                    // 导出 RAW
                    if ((options.ExportOption == ExportOptionEnum.RawOnly || options.ExportOption == ExportOptionEnum.Both) 
                        && !string.IsNullOrEmpty(photo.RawPath) && File.Exists(photo.RawPath))
                    {
                    ...
                    exportProgress.SuccessCount = successCount;
                }
                catch (Exception ex)
                {
